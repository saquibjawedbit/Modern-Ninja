...
public class Localization_SOURCE_Window : EditorWindow 
{
    ...
    private void OnGUI()
    {
        ...
        //If Localization Manager file selected & ready
        ...
        if (locManagerSelected)
        {
            ...
            {
                if (EditorUtility.DisplayDialog("Question", "You are about to reset the Localization Manager path... No file o file be removed, just the current Language Localization path will be removed from the registry. Are you sure to continue?", "Yes", "No"))
                {
                    PlayerPrefs.DeleteKey(locRegistryKey);
                    this.Close();
                    return;
